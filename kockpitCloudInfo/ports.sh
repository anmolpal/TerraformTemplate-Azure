...
while read i;
do
    echo "az vm open-port --resource-group kockpitNucleas --name "$machineName1" --port $i --priority $x" | sed "s/['\"]//g" >> /home/$USER/clouddrive/kockpitCloudInfo/list/port.>    echo $machinename1
    echo $i
    x=$(expr $x + 1)
done
